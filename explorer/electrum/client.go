...
type electrumClient struct {
	...
	// reached via explorerSvc.Stop(). Used to parent every per-cycle context
	// so they all die at shutdown, and to bound the requestTimeout context so
	// in-flight requests fail at shutdown rather than waiting their full 15 s.
	ctx    context.Context
...
func (c *electrumClient) reconnect() error {
	...
	// Fail all in-flight requests immediately so callers get "connection
	// closed" errors rather than waiting the full requestTimeout.
	c.pendMu.Lock()
	for id, ch := range c.pending {
		close(ch)
		delete(c.pending, id)
	...
	for {
		...
		// Replay subscriptions in parallel (bounded concurrency) so that a
		// wallet with many addresses does not stall recovery linearly.
		c.storedSubsMu.Lock()
		subs := make([]string, len(c.storedSubs))
		copy(subs, c.storedSubs)
		c.storedSubsMu.Unlock()

		const resubParallelism = 8
		sem := make(chan struct{}, resubParallelism)
		var resubWg sync.WaitGroup
		for _, sh := range subs {
			sh := sh
			sem <- struct{}{}
			resubWg.Add(1)
