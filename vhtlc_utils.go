...
// getSettleIntent returns the intent proof and message to register for settling a vhtlc within
// a batch session. A nil preimage selects the refund without receiver path, otherwise the claim
// path is used and the preimage is revealed as condition witness.
func getSettleIntent(session *batchSessionArgs, preimage []byte) (string, string, error) {
	var forfeitClosure script.Closure
	var settlementTapscript *waddrmgr.Tapscript
	var err error
	if preimage != nil {
		forfeitClosure = session.vhtlcScript.ClaimClosure
		settlementTapscript, err = session.vhtlcScript.ClaimTapscript()
	} else {
		forfeitClosure = session.vhtlcScript.RefundWithoutReceiverClosure
		settlementTapscript, err = session.vhtlcScript.RefundTapscript(false)
	}
	if err != nil {
		return "", "", fmt.Errorf("failed to get settlement tapscript for intent: %w", err)
	}

	vtxoLocktime, inputSequence := extractLocktimeAndSequence(forfeitClosure)

	inputs, tapLeaves, arkFields, err := getIntentInputs(
		session.vtxos, session.vhtlcScript, settlementTapscript, inputSequence,
	)
	if err != nil {
		return "", "", err
	}
...
func getIntentInputs(
	...
	taptreeField, err := txutils.VtxoTaprootTreeField.Encode(vhtlcScript.GetRevealedTapscripts())
	if err != nil {
