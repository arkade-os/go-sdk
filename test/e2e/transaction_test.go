...
func TestAutoSettle(t *testing.T) {
	...
	alice := setupClient(t, "", sdk.WithAutoSettle())
	offchainAddr, err := alice.NewOffchainAddress(ctx)
	require.NoError(t, err)

	// Subscribe to vtxo events BEFORE faucet so we observe both the VtxosAdded
	// (faucet) and the VtxoSettled / subsequent VtxosAdded (auto-settle)
	...
	fundedVtxo := faucetOffchain(t, alice, offchainAddr, 0.0001)
	...
	// Poll WhenNextSettlement up to 5 seconds: the auto-settle loop schedules
	// the timer asynchronously after the VtxosAdded event arrives, so the
	// schedule may not be visible the instant the faucet returns.
...
WAIT:
	...
	// After firing, WhenNextSettlement must reset to zero (until the next
	// VtxosAdded re-arms it). Be tolerant of a brief race where the loop is
	// racing the timer callback to clear the field.
	require.Eventually(t, func() bool {
