...
// lndInvoiceState returns the state ("OPEN", "SETTLED", ...) of the invoice with the given
// payment hash.
func lndInvoiceState(rHash string) (string, error) {
	out, err := runCommand(
		"docker", "exec", "lnd",
		"lncli", "--network=regtest",
		"lookupinvoice", rHash,
	...
	if err := json.Unmarshal([]byte(out), &resp); err != nil {
		return "", fmt.Errorf("parse lnd lookupinvoice response: %w (raw: %s)", err, out)
	}
