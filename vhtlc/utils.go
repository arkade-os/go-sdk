...
func parseClaimClosure(leaf string) (*script.ConditionMultisigClosure, error) {
	...
	if len(closure.PubKeys) != 2 {
		return nil, fmt.Errorf(
			"invalid claim closure: expected 2 pubkeys, got %d",
			len(closure.PubKeys),
		)
	}
	return &closure, nil
...
func parseRefundClosure(leaf string) (*script.MultisigClosure, error) {
	...
	if len(closure.PubKeys) != 3 {
		return nil, fmt.Errorf(
			"invalid refund closure: expected 3 pubkeys, got %d",
			len(closure.PubKeys),
		)
	}
	return &closure, nil
...
func parseUnilateralRefundWithoutReceiverClosure(leaf string) (*script.CSVMultisigClosure, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to decode unilateral refund without receiver closure: %w",
			err,
		)
	}
	closure := script.CSVMultisigClosure{}
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to parse unilateral refund without receiver closure: %w",
			err,
		)
	}
	if !ok {
