	...
	t.Run("Valid", func(t *testing.T) {
		for i, testCase := range fixtures.Valid {
			t.Run(fmt.Sprintf("Case_%d_%s", i, testCase["description"]), func(t *testing.T) {
				...
				refundWithoutReceiverDelayType := parseTimelockType(
					unilateralRefundWithoutReceiverDelay["type"].(string),
				)
				...
				script, err := vhtlc.NewVHTLCScriptFromOpts(vhtlc.Opts{
					...
					UnilateralClaimDelay: arklib.RelativeLocktime{
						Type:  claimDelayType,
						Value: uint32(unilateralClaimDelay["value"].(float64)),
					},
					UnilateralRefundDelay: arklib.RelativeLocktime{
						Type:  refundDelayType,
						Value: uint32(unilateralRefundDelay["value"].(float64)),
					},
					UnilateralRefundWithoutReceiverDelay: arklib.RelativeLocktime{
						Type:  refundWithoutReceiverDelayType,
						Value: uint32(unilateralRefundWithoutReceiverDelay["value"].(float64)),
					},
				})
				require.NoError(t, err)
				...
				for _, leaf := range taprootTree.LeafMerkleProofs {
					...
					require.Equal(
						t,
						expectedCtrlBlock["controlBlock"],
						hex.EncodeToString(tapTreeFullProof.ControlBlock),
					)
				}
	...
	t.Run("Invalid", func(t *testing.T) {
		for i, testCase := range fixtures.Invalid {
			t.Run(fmt.Sprintf("Case_%d_%s", i, testCase["description"]), func(t *testing.T) {
				...
				receiverPubKey, err := btcec.ParsePubKey(receiverBytes)
				require.NoError(t, err)
				receiverKey, err := schnorr.ParsePubKey(
					receiverPubKey.SerializeCompressed()[1:],
				) // Remove prefix byte
				require.NoError(t, err)
				...
				senderPubKey, err := btcec.ParsePubKey(senderBytes)
				require.NoError(t, err)
				senderKey, err := schnorr.ParsePubKey(
					senderPubKey.SerializeCompressed()[1:],
				) // Remove prefix byte
				require.NoError(t, err)
				...
				serverKey, err := schnorr.ParsePubKey(
					serverPubKey.SerializeCompressed()[1:],
				) // Remove prefix byte
				require.NoError(t, err)
...
func TestGetVhtlcScript(t *testing.T) {
	...
	require.Equal(
		t,
		schnorr.SerializePubKey(opts.Receiver),
		schnorr.SerializePubKey(derived.Receiver),
	)
	...
	require.Equal(
		...
		derived.UnilateralRefundWithoutReceiverDelay,
	)
	...
	require.Equal(
		t,
		hex.EncodeToString(refundWithoutReceiverScript),
		hex.EncodeToString(recoveredRefundWithoutReceiverScript),
	)
	require.Equal(
		t,
		hex.EncodeToString(unilateralClaimScript),
		hex.EncodeToString(recoveredUnilateralClaimScript),
	)
	require.Equal(
		t,
		hex.EncodeToString(unilateralRefundScript),
		hex.EncodeToString(recoveredUnilateralRefundScript),
	)
	require.Equal(
		t,
		hex.EncodeToString(unilateralRefundWithoutReceiverScript),
		hex.EncodeToString(recoveredUnilateralRefundWithoutReceiverScript),
	)
}
